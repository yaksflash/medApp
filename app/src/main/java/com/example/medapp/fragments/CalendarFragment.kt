...
class CalendarFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnOpenFamily.setOnClickListener {
            ...
            loadChildren() // Загружаем детей при открытии
        }

        btnCloseFamily.setOnClickListener {
            viewFamily.visibility = View.GONE
            viewCalendar.visibility = View.VISIBLE
            loadReminders() // Обновляем календарь при возврате (вдруг удалили ребенка)
        }

        // Настройка списка детей
        ...
        // Загрузка календаря по умолчанию
        loadReminders()
    }
    ...
    private fun loadReminders() {
        ...
        // Если это ребенок, скрываем кнопку "Семья"
    ...
    private fun showChildReminders(child: Child) {
        ...
        lifecycleScope.launch {
            ...
            val layout = LinearLayout(requireContext()).apply { orientation = LinearLayout.VERTICAL; setPadding(32, 16, 32, 16) }
            val scroll = android.widget.ScrollView(requireContext()) // Добавим скролл для длинных списков
                ...
                if (dayReminders.isNotEmpty()) {
                    ...
                    dayReminders.forEach { reminder ->
                        val reminderTextView = TextView(requireContext()).apply {
                            ...
                            setOnClickListener {
                                // Используем ту же логику редактирования, что и в календаре, но адаптированную
                                showReminderEditDialog(reminder, this)
                            }
                        }
    ...
    // Дублируем функцию редактирования для диалога ребенка (немного упрощенную или используем ту же)
    private fun showReminderEditDialog(reminder: Reminder, textView: TextView) {
        // Можно переиспользовать showEditDialog, но там логика обновления UI завязана на TextView
        // В данном случае это допустимо
        showEditDialog(reminder, textView)
    }
